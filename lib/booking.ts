// Breakdowns are not booked through the form: they are same-day jobs, so the
// site shows a phone number for those instead. The form covers pre-booked work.
export type Job = "transport" | "trackday" | "trade";
export type Timing = "date" | "flexible";
...
export const JOBS: Record<Job, { label: string; pickup: string; dropoff: string; details: string; placeholder: string }> = {
  transport: {
    ...
    placeholder: "1978 Bonneville bought at auction. Runs but I'd rather it went in the van. Seller is around weekdays.",
  },
  trackday: {
    label: "Track day",
...
export function validateBooking(raw: unknown): { ok: true; data: BookingInput } | { ok: false; errors: Record<string, string> } {
  ...
  const data: BookingInput = {
    job: (JOB_KEYS.includes(str("job") as Job) ? str("job") : "transport") as Job,
    name: str("name"),
    phone: str("phone"),
    bike: str("bike"),
    pickup: str("pickup"),
    dropoff: str("dropoff"),
    details: str("details"),
    timing: str("timing") === "flexible" ? "flexible" : "date",
    date: str("date"),
    website: str("website"),
  };
  ...
  if (data.dropoff.length < 3) errors.dropoff = "Where is it going?";
  ...
  if (data.timing === "date" && !/^\d{4}-\d{2}-\d{2}$/.test(data.date)) errors.date = "Pick a date, or tell us you're flexible.";
...
export function bookingAsText(b: BookingInput & { reference: string }, greeting = false): string {
  const when = b.timing === "flexible" ? "Flexible, whenever fits the route" : b.date;
  return [
